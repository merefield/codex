...
async fn pre_tool_use_blocks_apply_patch_before_execution() -> Result<()> {
    ...
    let mut builder = test_codex()
        .with_pre_build_hook(|home| {
            if let Err(error) =
                write_pre_tool_use_hook(home, Some("^Edit$"), "json_deny", "blocked edit")
            {
                panic!("failed to write pre tool use hook test fixture: {error}");
            }
        })
    ...
    let target = test.cwd_path().join(file_name);
    if target.exists() {
        fs::remove_file(&target).context("remove leftover apply_patch target")?;
    }

    test.submit_turn("run the blocked apply_patch").await?;
    ...
    assert!(
        output.contains("Tool blocked by PreToolUse hook: blocked edit. Tool: apply_patch"),
        "blocked apply_patch output should surface the hook reason",
    );
    assert!(
        !target.exists(),
        "blocked apply_patch should not modify the workspace"
    );
    ...
    assert_eq!(hook_inputs[0]["tool_use_id"], call_id);
    assert_eq!(hook_inputs[0]["tool_input"]["input"], patch);

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn pre_tool_use_does_not_fire_for_tools_without_structured_payloads() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    assert!(
        ...
        "tools without structured pre-tool-use payloads should not be blocked",
    );

    let hook_log_path = test.codex_home_path().join("pre_tool_use_hook_log.jsonl");
    assert!(
        !hook_log_path.exists(),
        "tools without structured pre-tool-use payloads should not trigger hooks",
    );
...
async fn post_tool_use_records_additional_context_for_apply_patch() -> Result<()> {
    ...
    test.submit_turn("run the apply_patch with post hook")
        .await?;
    ...
    assert_eq!(hook_inputs[0]["tool_use_id"], call_id);
    assert_eq!(hook_inputs[0]["tool_input"]["input"], patch);
    assert!(hook_inputs[0]["tool_response"].is_string());
    assert!(
        target.exists(),
        "apply_patch should still execute before post hook feedback"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn post_tool_use_does_not_fire_for_tools_without_structured_payloads() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    assert!(
        !output.contains("should not fire"),
        "tools without structured post-tool-use payloads should not be affected",
    );

    let hook_log_path = test.codex_home_path().join("post_tool_use_hook_log.jsonl");
    assert!(
        !hook_log_path.exists(),
        "tools without structured post-tool-use payloads should not trigger hooks",
    );
