...
pub(crate) struct BuiltinCommandFlags {
    ...
    pub(crate) repository_intelligence_command_enabled: bool,
...
pub(crate) fn builtins_for_input(flags: BuiltinCommandFlags) -> Vec<(&'static str, SlashCommand)> {
    built_in_slash_commands()
        ...
        .filter(|(_, cmd)| {
            flags.repository_intelligence_command_enabled || *cmd != SlashCommand::RepoIntel
        })
...
mod tests {
    ...
    fn all_enabled_flags() -> BuiltinCommandFlags {
        BuiltinCommandFlags {
            ...
            repository_intelligence_command_enabled: true,
    ...
    #[test]
    fn repository_intelligence_command_is_hidden_when_disabled() {
        let mut flags = all_enabled_flags();
        flags.repository_intelligence_command_enabled = false;
        assert_eq!(find_builtin_command("repo-intel", flags), None);
    }
